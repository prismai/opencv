...
public:
    ...
    Ptr<PluginWriter> create(const OpenCV_VideoIO_Plugin_API_preview* plugin_api,
            const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int bitrate)
    {
        CV_Assert(plugin_api);
        ...
        if (plugin_api->Writer_open)
        {
            ...
            if (CV_ERROR_OK == plugin_api->Writer_open(filename.empty() ? 0 : filename.c_str(), fourcc, fps, sz.width, sz.height, isColor, bitrate, &writer))
            {
                CV_Assert(writer);
...
Ptr<IVideoWriter> PluginBackend::createWriter(const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int bitrate) const
{
    try
    {
        if (plugin_api_)
            return PluginWriter::create(plugin_api_, filename, fourcc, fps, sz, isColor, bitrate); //.staticCast<IVideoWriter>();
    }
    catch (...)
    {
